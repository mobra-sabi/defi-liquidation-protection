#!/usr/bin/env python3
"""
Build CORRECT Dataset for Liquidation Prediction - NO LEAKAGE (V2)
Uses correct Aave V3 subgraph fields: atokenBalanceHistoryItems, vtokenBalanceHistoryItems, stokenBalanceHistoryItems
"""
...
def fetch_liquidations(limit=2000):
                ...
                if 'data' in data and data['data']:
                    ...
                    for liq in batch:
                        liquidations.append({
                            ...
                            'collateral_reserve_id': liq['collateralReserve']['id'],
                            'debt_reserve': liq['principalReserve']['symbol'],
                            'debt_reserve_id': liq['principalReserve']['id'],
                            'collateral_amount_raw': int(liq['collateralAmount']),
                            'debt_amount_raw': int(liq['principalAmount']),
                            'collateral_decimals': int(liq['collateralReserve']['decimals']),
                        })
                    ...
                    print(f"No data: {data.get('errors')}")
                    break
            else:
...
def fetch_balance_history(user_address, reserve_id, target_timestamp, history_type='atoken'):
    """Query balance history using the correct Aave V3 fields."""
    if history_type == 'atoken':
        query_field = 'atokenBalanceHistoryItems'
        balance_field = 'currentATokenBalance'
    elif history_type == 'vtoken':
        query_field = 'vtokenBalanceHistoryItems'
        balance_field = 'currentVariableDebt'
    elif history_type == 'stoken':
        query_field = 'stokenBalanceHistoryItems'
        balance_field = 'currentStableDebt'
    else:
        return None
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if 'data' in data and data['data']:
                items = data['data'].get(query_field, [])
                if items:
                    return items[0]
        return None
    except Exception as e:
...
def fetch_position_at_time(user_address, collateral_reserve_id, debt_reserve_id, target_timestamp):
    """Query position state at specific time."""
    # Fetch collateral (aToken) balance
    collateral = fetch_balance_history(user_address, collateral_reserve_id, target_timestamp, 'atoken')
    
    # Fetch variable debt
    variable_debt = fetch_balance_history(user_address, debt_reserve_id, target_timestamp, 'vtoken')
    
    # Fetch stable debt
    stable_debt = fetch_balance_history(user_address, debt_reserve_id, target_timestamp, 'stoken')
...
def build_sample(liq, time_offset, window_name):
    """Build a sample at specific time BEFORE liquidation."""
    try:
        user = liq['user']
        liq_timestamp = liq['timestamp']
        collateral_reserve_id = liq['collateral_reserve_id']
        debt_reserve_id = liq['debt_reserve_id']
        reserve_symbol = liq['collateral_reserve']
        
        observation_time = liq_timestamp - time_offset
        
        # Query position state
        position = fetch_position_at_time(user, collateral_reserve_id, debt_reserve_id, observation_time)
        
        if not position['collateral']:
            return None
        
        # Extract balances
        collateral_raw = int(position['collateral'].get('currentATokenBalance', 0))
        debt_var = int(position['variable_debt'].get('currentVariableDebt', 0)) if position['variable_debt'] else 0
        debt_stable = int(position['stable_debt'].get('currentStableDebt', 0)) if position['stable_debt'] else 0
        ...
        # Get reserve info from collateral data
        reserve = position['collateral'].get('userReserve', {}).get('reserve', {})
        ...
        return {
            ...
            'debt_raw': debt_raw,
            'debt_variable': debt_var,
            'debt_stable': debt_stable,
            ...
            'base_ltv': base_ltv,
            'price_usd_at_observation': price_usd,
...
def build_correct_dataset():
    """Build the correct dataset."""
    print("\n" + "="*70)
    print("BUILDING CORRECT DATASET - NO LEAKAGE")
    print("="*70)
    
    # Step 1: Get liquidations
    liquidations = fetch_liquidations(limit=2000)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(build_sample, liq, offset, name): (liq, offset, name) 
    ...
    print(f"  Class 0 (survives 30min): {class_0:,} ({class_0/len(df):.1%})")
    print(f"  Class 1 (liquidated in 30min): {class_1:,} ({class_1/len(df):.1%})")
